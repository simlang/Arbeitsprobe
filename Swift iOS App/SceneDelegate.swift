...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        if let appDelegate = UIApplication.shared.delegate as? AppDelegate {
            surveyModel = appDelegate.surveyModel
            userDefaultsPublisher = appDelegate.userDefaultsPublisher
            apiRequestManager = appDelegate.apiRequestManager
        }
        ...
        if let windowScene = scene as? UIWindowScene {
            ...
            window.rootViewController = UIHostingController(rootView:
                contentView
                    .environmentObject((surveyModel ?? SurveyModel()))
                    .environmentObject(ActivityModel())
                    .environmentObject(HomescreenViewRouter())
                    .environmentObject(UserProfileModel())
                    .environmentObject((userDefaultsPublisher ?? UserDefaultsPublisher())))
            self.window = window
    ...
    func sceneDidEnterBackground(_ scene: UIScene) {
        ...
        UserProfileModel.updatedStats = false
    }
}
